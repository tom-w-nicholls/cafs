...
"""
***Currently not used*** because calculation times are too long
 
/***************************************************************************
 LargeAreaShadowAlgorithm
  (even though it is marginally more accurate)
 Calculates shadows of a geographical region based on obstructions
 Similar to wide area calculation but runs over a larger area
...
class LargeAreaShadowAlgorithm(QgsProcessingAlgorithm):
    """
    """
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        csvOutputFilePath = resultsPath / (CSV_SHADOW_OUTPUT_FILENAME + '.csv')
        ...
        try:
            ...
            shadowRoofsPath = resultsPath / (SHADOW_REJECTED_ROOFS_LAYER_NAME + '.shp')
            log(f"Suitable Roofs Path = {shadowRoofsPath}")

            writer = QgsVectorFileWriter(str(shadowRoofsPath), 
                                    getSystemEncoding(),
                                    newFields,
                                    QgsWkbTypes.Polygon, 
                                    sourceCrs,
            ...
            with open(csvOutputFilePath, 'w') as outputFile:
                ...
                for current, feature in enumerate(features):
                    ...
                    if geom: # only if not null, otherwise don't include this feature in the new source
                        ...
                        # IN THIS CASE FIND ROOFS THAT ARE NOT CONSIDERED WORTHY BUT MIGHT HAVE BEEN ELIMINATED UNNECESSARILY
                        # i.e. roos that have shadow less than 0.5 but are worthy in all other respects
                        if(areaTrue>MINIMUM_USEFUL_AREA and slopeMean <= 60 and (not aspectFilter) and shadeMean<0.5):
                            # Do a calculation shadeMean * areaTrue and see if it is larger than MINIMUM AREA
                            # This is a very rough calculation - will be a smallish number of roofs
                            if(shadeMean * areaTrue > MINIMUM_USEFUL_AREA): # Roofs marginally rejected due to being partially in shadow but still possibly valid
        ...
        #                         newFeature.setAttribute(idxAreaPlan, geom.area()) 
            ...
            #                     Step 95: keep a running total of the total energy output for the entire area
                                ...
                                roofOutput = REAL_PANEL_AREA * shadeMean * integerNumberOfPanels * PANEL_YIELD * PANEL_EFFICIENCY * energyOverThisRoof
...
#                     TODO: Add in steps 82, 85, 90, 92, 95 here in the loop and find a total irradiation for the entire area
    ...
    def shortHelpString(self):
        return self.tr("Extra CAFS algorithm: Find roofs in shadow that might be large enough to consider")

    def createInstance(self):
        return LargeAreaShadowAlgorithm()
